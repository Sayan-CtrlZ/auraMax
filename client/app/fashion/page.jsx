...
export default function FashionPage() {
  ...
  const handleUseSample = () => {
    setSelectedImage("/sample-full-body.webp");
  };
  ...
  const handleGenerate = async () => {
    ...
    const itemsToSave = MOCK_RESULTS.map((res) => ({
      title: res.title,
      links: res.links
    }));
    ...
    addHistoryItem({
      ...
      details: {
        weather: weather,
        mood: styleVibes.map(vId => STYLE_VIBES.find(v => v.id === vId)?.name).join(", ") || "Casual",
        occasion: occasion,
        budget: budget,
        items: itemsToSave,
        advice: "Focus on rich textures and bold silhouettes to maximize visual impact for this occasion. Opt for cohesive accessories."
      }
    });
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-[#FAF6F0] pt-28 pb-16 px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                      ...
                      {selectedImage ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <span className="text-[10px] text-stone-400 font-bold px-2 uppercase tracking-widest">or</span>
                            <span className="h-px bg-stone-200 flex-1"></span>
                          </div>

                          <button 
                            onClick={handleUseSample}
                            className="w-full py-3 bg-stone-100/80 hover:bg-stone-200 text-stone-700 font-medium rounded-xl text-xs transition-colors flex items-center justify-center space-x-2"
                          >
          ...
          <div className={cn("transition-all duration-700", results ? "lg:col-span-12 w-full max-w-6xl mx-auto" : "lg:col-span-7")}>
            ...
            {results && !isLoading && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {results.map((outfit) => (
                    <div key={outfit.id} className="bg-white rounded-2xl border border-stone-200/60 overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col">
                      {/* Generated Image */}
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto space-y-3">
                          <h4 className="text-[10px] font-bold text-stone-400 uppercase tracking-widest flex items-center space-x-1.5">
                            <ShoppingBag size={12} />
                            <span>Shop The Look</span>
                          </h4>
                          <ul className="space-y-2">
                            {outfit.links.map((link, idx) => (
                              <li key={idx}>
                                <a 
                                  href={link.url} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="group/link flex items-center justify-between p-2 rounded-lg bg-stone-50 hover:bg-brand-rose/5 border border-stone-100 hover:border-brand-rose/20 transition-all"
                                >
                                  <span className="text-xs text-stone-600 font-medium group-hover/link:text-brand-rose transition-colors line-clamp-1">
                                    {link.name}
                                  </span>
                                  <ExternalLink size={12} className="text-stone-400 group-hover/link:text-brand-rose flex-shrink-0" />
                                </a>
                              </li>
                            ))}
                          </ul>
